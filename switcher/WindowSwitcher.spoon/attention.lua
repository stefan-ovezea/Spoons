...
local function logNotification(state, element, stackingID, notification)
    ...
    state.logger.df(
        "notification system message: event=%s summary={%s} staticText=%s",
        tostring(notification),
        elementSummary(element),
        table.concat(texts, " | ")
    )
    state.logger.df(
        "notification parsed payload: identifier=%s stackingID=%s parsedBundleID=%s",
...
local function handleNotificationElement(state, element, depth, notification)
    if not element then return end
    ...
    if not notificationSubroles[subrole] then
        ...
        for _, child in ipairs(children) do
            handleNotificationElement(state, child, (depth or 0) + 1, notification)
        end
    ...
    logNotification(state, element, stackingID, notification)
...
local function startNotificationObserver(state)
    ...
    local observer = hs.axuielement.observer
        ...
        :callback(function(_, element, notification)
            ...
            handleNotificationElement(state, element, 0, notification)
        end)
